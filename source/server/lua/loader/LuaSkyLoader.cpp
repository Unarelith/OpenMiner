...
void LuaSkyLoader::loadObjects(Sky &sky, const sol::table &table) const {
	if (sol::object obj = table["objects"].get<sol::object>() ; obj.valid()) {
		sol::table objectsTable = obj.as<sol::table>();

		if (sol::object obj = objectsTable["sun"].get<sol::object>() ; obj.valid()) {
			...
			sunDefinition.size = sunTable["size"].get_or(512.f);

			sky.setSunDefinition(sunDefinition);
		}

		if (sol::object obj = objectsTable["moon"].get<sol::object>() ; obj.valid()) {
			...
			moonDefinition.size = moonTable["size"].get_or(512.f);

			if (sol::object obj = moonTable["phases"].get<sol::object>() ; obj.valid()) {
				sol::table phasesTable = obj.as<sol::table>();
				moonDefinition.phaseCount = phasesTable["count"].get_or<u16>(1);
				moonDefinition.phaseSize = phasesTable["size"].get_or<u16>(32);
			}

			sky.setMoonDefinition(moonDefinition);
		}

		if (sol::object obj = objectsTable["stars"].get<sol::object>() ; obj.valid()) {
			sol::table starsTable = obj.as<sol::table>();

			Sky::StarsDefinition starsDefinition;
			starsDefinition.count = starsTable["count"].get_or<u16>(1000);
			starsDefinition.size = starsTable["size"].get_or<u16>(4);
